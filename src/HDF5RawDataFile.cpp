...
namespace dunedaq
{
  namespace hdf5libs
  {
        ...
        : m_bare_file_name(file_name), m_open_flags(open_flags)
    {
      ...
      // // initialize MPI
      // HighFive::MPI_Init(&argc, &argv);
      // HighFive::MPI_Comm_size(HighFive::MPI_COMM_WORLD, &mpi_size);
      // HighFive::MPI_Comm_rank(HighFive::MPI_COMM_WORLD, &mpi_rank);
      ...
      HighFive::FileAccessProps file_access_props = HighFive::FileAccessProps::Default();
      // file_access_props.add(HighFive::MPIOFileAccess{HighFive::MPI_COMM_WORLD, HighFive::MPI_INFO_NULL});
      // file_access_props.add(HighFive::MPIOCollectiveMetadata{});

      // do the file open
      try
      {
        m_file_ptr.reset(new HighFive::File(filename_to_open, m_open_flags, file_access_props));
      }
    ...
    /**
     * @brief Write a trigger record to the file with separated fragment buffer.
     * Need to finish with write_end_tr function
     */
    void
    HDF5RawDataFile::write_header_only(const daqdataformats::TriggerRecord &tr, HDF5SourceIDHandler::source_id_path_map_t &source_id_path_map, HighFive::Group &record_level_group)
    {

      // write the record header into the HDF5 file/group
      record_level_group = write(tr.get_header_ref(), source_id_path_map);
    ...
    /**
     * @brief Write a Fragment to the file depending of the current trigger record.
     */
    void
    HDF5RawDataFile::write_fragment_only(std::unique_ptr<daqdataformats::Fragment> &frag_ptr, HDF5SourceIDHandler::source_id_path_map_t &source_id_path_map, HDF5SourceIDHandler::fragment_type_source_id_map_t &fragment_type_source_id_map, HDF5SourceIDHandler::subdetector_source_id_map_t &subdetector_source_id_map, bool allocate_early)
    {

      // write the fragment into the HDF5 file/group
     ...
     */
    void
    HDF5RawDataFile::write_fragment_thread(daqdataformats::FragmentHeader &fh,
                                           ...
                                           bool allocate_early)
    {
      ...
      std::vector<std::string> const &group_and_dataset_path_elements = m_file_layout_ptr->get_path_elements(fh);
      ...
      auto f = [](HighFive::DataSet &data_set, char *buffer, long unsigned int start_index, long unsigned int write_size)
      {
        data_set.select({start_index, 0}, {write_size, 1}).write_raw(buffer);
      };
      ...
      auto chunk_num = raw_data_size_bytes / chunk_size;
      ...
      for (int i = 0; i < chunk_num; i++)
      {
        ...
        {
          threads[started_threads] = std::thread(f, std::ref(data_set), buffer, i * chunk_size, chunk_size);
          started_threads++;
        }
        else
        {
      ...
      m_recorded_size += raw_data_size_bytes;

      daqdataformats::SourceID source_id = fh.element_id;
      HDF5SourceIDHandler::add_source_id_path_to_map(source_id_path_map, source_id, data_set_path);

      HDF5SourceIDHandler::add_fragment_type_source_id_to_map(
          fragment_type_source_id_map, static_cast<daqdataformats::FragmentType>(fh.fragment_type), fh.element_id);
      HDF5SourceIDHandler::add_subdetector_source_id_to_map(
          subdetector_source_id_map,
          static_cast<detdataformats::DetID::Subdetector>(fh.detector_id),
          fh.element_id);
    }

    /**
     * @brief Write end of trigger record to the file.
     */
    void
    HDF5RawDataFile::write_end_tr(HDF5SourceIDHandler::source_id_path_map_t &source_id_path_map, HighFive::Group &record_level_group, HDF5SourceIDHandler::fragment_type_source_id_map_t &fragment_type_source_id_map, HDF5SourceIDHandler::subdetector_source_id_map_t &subdetector_source_id_map)
    {
    ...
    /**
     * @brief Write a new Fragment to the file, only begining of the fragment, need to write parts after.
     */
    HighFive::DataSet
    HDF5RawDataFile::write_fragment_begin(daqdataformats::FragmentHeader &fh, size_t raw_data_size_bytes, size_t chunk_size, bool allocate_early)
    {
      std::vector<std::string> const &group_and_dataset_path_elements = m_file_layout_ptr->get_path_elements(fh);
      const std::string dataset_name = group_and_dataset_path_elements.back();
    ...
    /**
     * @brief Write a Fragment part to the file
     */
    void
    HDF5RawDataFile::write_fragment_part(HighFive::DataSet &data_set, const char *buffer, const long unsigned int start_index, const long unsigned int write_size)
    {

      if (data_set.isValid())
      {
        // if (start_index != 0)
        //   data_set.resize({end_index * (start_index + 1), 1});

        data_set.select({start_index, 0}, {write_size, 1}).write_raw(buffer);

        m_recorded_size += write_size;
      }
      else
      {
        throw InvalidHDF5Dataset(ERS_HERE, "", m_file_ptr->getName());
      }
    }

    /**
     * @brief Write the end of the fragment
     */
    void
    HDF5RawDataFile::write_fragment_end(HDF5SourceIDHandler::fragment_type_source_id_map_t &fragment_type_source_id_map, HDF5SourceIDHandler::subdetector_source_id_map_t &subdetector_source_id_map, HDF5SourceIDHandler::source_id_path_map_t &source_id_path_map, std::string data_set_path, daqdataformats::FragmentHeader &fh)
    {
      m_file_ptr->flush();
     ...
     */
    void
    HDF5RawDataFile::write(const daqdataformats::Fragment &frag, HDF5SourceIDHandler::source_id_path_map_t &path_map, bool allocate_early)
    {
      std::tuple<size_t, std::string, HighFive::Group> write_results =

 ...
 */
std::set<HDF5RawDataFile::record_id_t> // NOLINT(build/unsigned)
...
{
        ers::warning(DeprecatedUsage(ERS_HERE,"get_all_record_numbers()",
...
std::set<HDF5RawDataFile::record_id_t>
...
{
        ers::warning(DeprecatedUsage(ERS_HERE,"get_all_trigger_record_numbers()",
...
HDF5RawDataFile::get_record_header_dataset_paths()
{
        ...
        if(get_version()>=2) {
                ...
                for (auto const& rec_id : get_all_record_ids())
                        rec_paths.push_back(m_file_ptr->getPath() + m_file_layout_ptr->get_record_header_path(rec_id.first,rec_id.second));
...
std::string HDF5RawDataFile::get_record_header_dataset_path(const record_id_t rid)
{
        ...
        return (m_file_ptr->getPath() + m_file_layout_ptr->get_record_header_path(rec_id->first,rec_id->second));
}
...
std::vector<std::string> HDF5RawDataFile::get_fragment_dataset_paths(const record_id_t rid)
{
        ...
        std::string record_group_path = m_file_ptr->getPath()+m_file_layout_ptr->get_record_number_string(rec_id->first,rec_id->second);
...
//get all fragment dataset paths for given record ID
std::vector<std::string> HDF5RawDataFile::get_fragment_dataset_paths(const uint64_t rec_num, // NOLINT (build/unsigned)
                                                                     const daqdataformats::sequence_number_t seq_num)
{
...
std::vector<std::string>
HDF5RawDataFile::get_fragment_dataset_paths(const daqdataformats::GeoID::SystemType type)
{
        std::vector<std::string> frag_paths;
        for( auto const& rid : get_all_record_ids()) {
                auto datasets = get_dataset_paths(m_file_ptr->getPath() + m_file_layout_ptr->get_fragment_type_path(rid.first,rid.second,type));
...
{
        daqdataformats::GeoID::SystemType type = daqdataformats::GeoID::string_to_system_type(typestring);
                                            ...
                                            const daqdataformats::GeoID::SystemType type)
{
  ...
  return get_dataset_paths(m_file_ptr->getPath() + m_file_layout_ptr->get_fragment_type_path(rid.first,rid.second,type));
}
...
std::vector<std::string>
HDF5RawDataFile::get_fragment_dataset_paths(const daqdataformats::GeoID element_id)
{
        std::vector<std::string> frag_paths;

        for( auto const& rid : get_all_record_ids())
                frag_paths.push_back(m_file_ptr->getPath()+m_file_layout_ptr->get_fragment_path(rid.first,rid.second,element_id));
...
{
        return get_fragment_dataset_paths(daqdataformats::GeoID(type,region_id,element_id));
}
std::vector<std::string>
HDF5RawDataFile::get_fragment_dataset_paths(const std::string typestring,
                                            const uint16_t region_id,         //NOLINT(build/unsigned)
                                            const uint32_t element_id)         //NOLINT(build/unsigned)
{
        return get_fragment_dataset_paths(daqdataformats::GeoID(daqdataformats::GeoID::string_to_system_type(typestring),region_id,element_id));
}


std::set<daqdataformats::GeoID> HDF5RawDataFile::get_geo_ids(std::vector<std::string> const& frag_dataset_paths)
{
  std::set<daqdataformats::GeoID> geo_ids;
  ...
  for(auto const& frag_dataset : frag_dataset_paths){
    ...
    geo_ids.insert(m_file_layout_ptr->get_geo_id_from_path_elements(path_elements));
  }
                              ...
                              const uint32_t element_id) // NOLINT(build/unsigned)
{
        ...
        return get_frag_ptr(m_file_layout_ptr->get_fragment_path(rid.first, rid.second, type, region_id, element_id));
}
                              ...
                              const uint32_t element_id) // NOLINT(build/unsigned)
{
        ...
        return get_frag_ptr(m_file_layout_ptr->get_fragment_path(rid.first, rid.second, typestring, region_id, element_id));
}

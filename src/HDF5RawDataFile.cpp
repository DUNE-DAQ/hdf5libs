...
HDF5RawDataFile::write(const daqdataformats::TriggerRecordHeader& trh)
{
  ...
  m_recorded_size +=
    do_write(get_path_elements(trh), static_cast<const char*>(trh.get_storage_location()), trh.get_total_size_bytes());
}
...
HDF5RawDataFile::write(const daqdataformats::Fragment& frag)
{
  ...
  m_recorded_size += do_write(
    get_path_elements(frag.get_header()), static_cast<const char*>(frag.get_storage_location()), frag.get_size());
}
...
template<typename T>
void
HDF5RawDataFile::write_attribute(HighFive::Group* grp_ptr, std::string name, T value)
{
...
template<typename T>
void
HDF5RawDataFile::write_attribute(HighFive::DataSet* d_ptr, std::string name, T value)
{
...
HDF5RawDataFile::write_file_layout()
{
  ...
  if (!m_file_ptr->exist("DUNEDAQFileLayout"))
    m_file_ptr->createGroup("DUNEDAQFileLayout");
...
HDF5RawDataFile::get_path_elements(const daqdataformats::TriggerRecordHeader& trh)
{
 ...
 */
std::vector<std::string>
HDF5RawDataFile::get_path_elements(const daqdataformats::FragmentHeader& fh)
{
 ...
 */
std::string
HDF5RawDataFile::get_trigger_number_string(daqdataformats::trigger_number_t trig_num, daqdataformats::sequence_number_t)
{
...
template<typename T>
T
HDF5RawDataFile::get_attribute(std::string name)
{
  if (!m_file_ptr->hasAttribute(name)) {
...
template<typename T>
T
HDF5RawDataFile::get_attribute(HighFive::Group* grp_ptr, std::string name)
{
  if (!(grp_ptr->hasAttribute(name))) {
    // throw that we don't have that attribute
  }
...
template<typename T>
T
HDF5RawDataFile::get_attribute(HighFive::DataSet* d_ptr, std::string name)
{
  if (!d_ptr->hasAttribute(name)) {
    // throw that we don't have that attribute
  }
...
HDF5RawDataFile::read_file_layout()
{
  ...
  for (auto const& oname : object_names) {
    if (fl_group.getObjectType(oname) != HighFive::ObjectType::Group)
      continue;
 ...
 */
std::vector<std::string>
HDF5RawDataFile::get_fragments(const unsigned& start_tr, const unsigned& num_trs)
{
 ...
 */
std::vector<std::string>
HDF5RawDataFile::get_trh(const unsigned& start_tr, const unsigned& num_trs)
{
  ...
  unsigned int trs_count = 0;
  for (auto& element : dataset_path) {
    if (element.find("TriggerRecordHeader") != std::string::npos && trs_count < start_tr + num_trs &&
        trs_count >= start_tr) {
...
HDF5RawDataFile::get_attributes()
{
...
HDF5RawDataFile::get_frag_ptr(const std::string& dataset_name)
{
  ...
  std::unique_ptr<daqdataformats::Fragment> frag(new dunedaq::daqdataformats::Fragment(
    membuffer, dunedaq::daqdataformats::Fragment::BufferAdoptionMode::kTakeOverBuffer));
  // delete[] membuffer;
...
HDF5RawDataFile::get_trh_ptr(const std::string& dataset_name)
{
  ...
  char* membuffer = new char[data_size];
  data_set.read(membuffer);
  std::unique_ptr<daqdataformats::TriggerRecordHeader> trh(
    new dunedaq::daqdataformats::TriggerRecordHeader(membuffer, true));
  delete[] membuffer;
  return std::move(trh);
}

...
namespace dunedaq
{
  ...
  namespace hdf5libs
  {
    ...
    public:
      void write_header_only(const daqdataformats::TriggerRecord &tr, HDF5SourceIDHandler::source_id_path_map_t &source_id_path_map, HighFive::Group &record_level_group);
      ...
      HighFive::DataSet write_fragment_begin(daqdataformats::FragmentHeader &fh, size_t raw_data_size_bytes, size_t chunk_size, bool allocate_early = false);
    ...
    private:
      ...
      HighFive::Group write(const daqdataformats::TimeSliceHeader &tsh,
                            HDF5SourceIDHandler::source_id_path_map_t &path_map);
      void write(const daqdataformats::Fragment &frag, HDF5SourceIDHandler::source_id_path_map_t &path_map, bool allocate_early = false);
  ...
  std::set<daqdataformats::SourceID> get_source_ids(const std::string& typestring)
  {
    ...
    private:
      ...
      // writing to datasets
      std::tuple<size_t, std::string, HighFive::Group> do_write(std::vector<std::string> const &, const char *, size_t, bool allocate_early = false);

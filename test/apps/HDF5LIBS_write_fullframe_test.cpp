...
void print_usage()
{
  TLOG() << "Usage: filetransform_file_to_file_test <configuration_file> <detector_readout_map_file> <input_file_name> <output_file_name>";
}
...
int main(int argc, char **argv)
{
  ...
  // read fragment data
  // const int buffer_size = (argc == 6) ? std::stoi(argv[5]) : data_size;
  ...
  for (int trig_num = 1; trig_num <= trigger_count; ++trig_num)
  {
    ...
    for (int ele_num = 0; ele_num < element_count; ++ele_num)
    {
      ...
      auto frag_ptr = std::make_unique<Fragment>(buffer, data_size);
      frag_ptr->set_header_fields(fh);

      h5_raw_data_file.write_fragment_only(frag_ptr, source_id_path_map, fragment_type_source_id_map, subdetector_source_id_map, true);
